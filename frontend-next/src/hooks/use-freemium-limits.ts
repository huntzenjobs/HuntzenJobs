...
export const PLAN_LIMITS = {
  ...
  starter: {
    ...
    coach_minutes_per_day: 30,  // 1800 seconds in DB = 30 minutes
  ...
  pro: {
    ...
    coach_minutes_per_day: Infinity,  // -1 in DB = unlimited
    has_advanced_filters: true,
    has_favorites: true,
    has_email_alerts: false,
    has_visual_score: true,
...
function getDefaultState(): FreemiumState {
  return {
    ...
    usage: {
      ...
      jobsViewedToday: 0,
      cvAnalysesToday: 0,
      coachSecondsUsedToday: 0,
      coachSessionStartTime: null,
...
export function useFreemiumLimits() {
  ...
  const canUse = useCallback(
    (feature: FeatureType): boolean => {
      ...
      switch (feature) {
        case 'job_search':
          return currentState.usage.searchesToday < currentLimits.job_searches_per_day
        case 'job_view':
          return currentState.usage.jobsViewedToday < currentLimits.jobs_visible
        case 'cv_analysis':
          return currentState.usage.cvAnalysesToday < currentLimits.cv_analyses_per_day
        case 'coach_time':
          return currentState.usage.coachSecondsUsedToday < currentLimits.coach_minutes_per_day * 60
        default:
          return false
      }
    },
  ...
  const getRemaining = useCallback(
    (feature: FeatureType): number => {
      ...
      switch (feature) {
        case 'job_search':
          return Math.max(0, currentLimits.job_searches_per_day - currentState.usage.searchesToday)
        case 'job_view':
          return Math.max(0, currentLimits.jobs_visible - currentState.usage.jobsViewedToday)
        case 'cv_analysis':
          return Math.max(0, currentLimits.cv_analyses_per_day - currentState.usage.cvAnalysesToday)
        case 'coach_time':
          return Math.max(
            0,
            currentLimits.coach_minutes_per_day * 60 - currentState.usage.coachSecondsUsedToday
          )
        default:
          return 0
      }
      ...
      setState((prev) => {
        ...
        switch (feature) {
          ...
          case 'cv_analysis':
            ...
            break
          case 'coach_time':
            newState.usage.coachSecondsUsedToday += amount
            break
        }
        saveState(newState)
        return newState
  ...
  // Start coach session timer
  const startCoachSession = useCallback((): void => {
    setState((prev) => {
      if (prev.usage.coachSessionStartTime !== null) return prev
      const newState = {
        ...prev,
        usage: {
          ...prev.usage,
          coachSessionStartTime: Date.now(),
        },
      }
      saveState(newState)
      return newState
  ...
  // Stop coach session and record time
  const stopCoachSession = useCallback((): void => {
    setState((prev) => {
      if (prev.usage.coachSessionStartTime === null) return prev
      const elapsed = Math.round((Date.now() - prev.usage.coachSessionStartTime) / 1000)
      const newState = {
        ...prev,
        usage: {
          ...prev.usage,
          coachSecondsUsedToday: prev.usage.coachSecondsUsedToday + elapsed,
          coachSessionStartTime: null,
        },
      }
      saveState(newState)
  ...
  // Get coach time remaining in seconds (accounting for active session) - stable reference using ref
  const getCoachTimeRemaining = useCallback((): number => {
    const currentState = stateRef.current
    const currentLimits = limitsRef.current

    const totalAllowed = currentLimits.coach_minutes_per_day * 60
    let used = currentState.usage.coachSecondsUsedToday

    // Add time from active session
    if (currentState.usage.coachSessionStartTime !== null) {
      used += Math.round((Date.now() - currentState.usage.coachSessionStartTime) / 1000)
    }
  ...
  const usage = useMemo(() => state.usage, [
    state.usage.searchesToday,
    state.usage.jobsViewedToday,
    state.usage.cvAnalysesToday,
    state.usage.coachSecondsUsedToday,
    state.usage.coachSessionStartTime,
  ...
  return {
    ...
    startCoachSession,
    stopCoachSession,
    getCoachTimeRemaining,
    isCoachSessionActive: state.usage.coachSessionStartTime !== null,
  }
}
