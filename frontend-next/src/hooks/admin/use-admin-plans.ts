...
export function useAdminPlans() {
  ...
  const updateLimits = useCallback(async (planId: string, limits: Partial<PlanLimit>) => {
    setLoading(true)
    try {
  ...
  const updateFeatures = useCallback(async (planId: string, features: string[]) => {
    setLoading(true)
    try {
  ...
  const updateStripePrice = useCallback(async (
    planId: string,
    billing_period: 'monthly' | 'yearly',
    unit_amount: number,
    currency = 'eur'
    ...
    setLoading(true)
    try {
      const result = await adminFetch(`/api/admin/plans/${planId}/stripe-price`, {
        method: 'POST',
  ...
  return { loading, fetchPlans, updateLimits, updateFeatures, updateDisplayPrice, updateStripePrice }
}
