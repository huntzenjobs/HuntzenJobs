...
export function SettingsSection({
  ...
  // State for settings
  const [language, setLanguage] = useState(initialSettings.preferred_language || 'fr')
  ...
  useEffect(() => {
    // Skip initial render
    if (debouncedEmailNotifications === (initialSettings.email_notifications ?? true)) {
      return
    }

    const save = async () => {
      setIsUpdatingNotifications(true)
      const success = await updateSettings('email_notifications', debouncedEmailNotifications)
    ...
    save()
  }, [debouncedEmailNotifications, initialSettings.email_notifications, updateSettings])

  // Auto-save newsletter
  useEffect(() => {
    // Skip initial render
    if (debouncedNewsletter === (initialSettings.newsletter_subscribed ?? false)) {
      return
    }

    const save = async () => {
      setIsUpdatingNewsletter(true)
      const success = await updateSettings('newsletter_subscribed', debouncedNewsletter)
    ...
    save()
  }, [debouncedNewsletter, initialSettings.newsletter_subscribed, updateSettings])
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        <p className="text-xs text-gray-500 bg-gray-50 border border-gray-200 rounded-lg p-3">
          💡 Le support multilingue sera disponible prochainement
        </p>
      </div>
          ...
          <div className="flex items-start gap-2 flex-1">
            ...
            <div className="space-y-1">
              ...
              <p id="email-notifications-description" className="text-sm text-gray-500">
                Recevez des notifications pour les événements importants (nouvelles offres
                correspondantes, analyses CV terminées, messages importants)
              </p>
            </div>
          </div>
      ...
      <div className="space-y-4">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-2 flex-1">
            ...
            <div className="space-y-1">
              <Label htmlFor="newsletter" className="text-base font-semibold cursor-pointer">
                Newsletter HuntZen
              </Label>
              <p id="newsletter-description" className="text-sm text-gray-500">
                Recevez nos conseils exclusifs, astuces de recherche d'emploi, et les
                dernières fonctionnalités (1 email par semaine maximum)
              </p>
            </div>
          </div>
        ...
        {newsletter && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm">
            ...
            <p className="text-blue-700">
              Vous recevrez notre prochaine newsletter avec des conseils exclusifs pour
              maximiser vos chances de décrocher le job de vos rêves.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="pt-6 border-t">
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 text-sm text-gray-600 space-y-2">
          <p className="font-medium text-gray-900">🔒 Confidentialité et données</p>
          <p>
            Vos préférences sont enregistrées de manière sécurisée. Vous pouvez modifier
            ces paramètres à tout moment. Nous ne partageons jamais vos informations avec
      ...
      <div className="pt-6 border-t space-y-6">
        ...
        <div className="pt-4 border-t border-red-200">
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 space-y-4">
            <div>
              <h4 className="font-semibold text-red-900 mb-2">⚠️ Zone de danger</h4>
            ...
            <p className="text-xs text-red-600">
              💡 La suppression de compte sera bientôt disponible (RGPD compliance)
            </p>
          </div>
        </div>
      </div>
