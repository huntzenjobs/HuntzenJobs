...
// Category labels are now in i18n: cv.suggestions.categories.*
...
// getCategoryLabel is now handled via useTranslations in the component
...
export function ActionableSuggestions({
  ...
  const t = useTranslations("cv.suggestions");
  ...
  const stats = useMemo(() => {
    ...
    const completionRate =
      totalSuggestions > 0 ? (completedCount / totalSuggestions) * 100 : 0;
      ...
      <div className="bg-gradient-to-br from-blue-50 to-violet-50 rounded-xl p-5 border border-blue-200">
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="text-right">
            ...
            <div className="text-xs text-gray-600">{t("potentialScore")}</div>
          </div>
        </div>
        ...
        {stats.scoreGain > 0 && (
          <p className="text-sm text-blue-700 font-medium mt-3">
            {t("possiblePoints", { points: stats.scoreGain })}
          </p>
        )}
      </div>
