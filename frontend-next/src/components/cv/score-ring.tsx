...
export function ScoreRing({
  ...
  const getColor = (s: number) => {
    if (s >= 70)
      return { stroke: "#22c55e", bg: "from-green-500 to-emerald-500" };
    if (s >= 50)
      return { stroke: "#f59e0b", bg: "from-amber-500 to-orange-500" };
...
export function MatchingScore({ score, className = "" }: MatchingScoreProps) {
  const getMatchLevel = (s: number) => {
    if (s >= 80)
      return { label: "Excellent match", color: "text-green-600", emoji: "✨" };
    if (s >= 60)
      return { label: "Bon match", color: "text-amber-600", emoji: "👍" };
    if (s >= 40)
      return { label: "Match moyen", color: "text-orange-600", emoji: "🤔" };
