...
export default function AdminPlansPage() {
  const { fetchPlans, updateLimits, updateFeatures, updateDisplayPrice, updateStripePrice, loading } = useAdminPlans()
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Éditeur de packages</h1>
        <p className="text-muted-foreground text-sm mt-1">
          Modifiez les limites, fonctionnalités et prix de chaque plan. Les changements sont immédiats.
        </p>
      </div>
          ...
          {plans.map(plan => (
            <PlanCardEditor
              ...
              onUpdateStripePrice={async (id, period, amount, currency) => {
                const result = await updateStripePrice(id, period, amount, currency)
                if (result) refresh()
                return result
              }}
            />
