...
// Mock i18n context (custom useLocale from i18n-context.tsx)
vi.mock("@/contexts/i18n-context", () => ({
  useLocale: () => "fr",
  I18nProvider: ({ children }: { children: React.ReactNode }) => children,
}));

// Mock language switcher (uses custom useLocale)
vi.mock("@/components/language-switcher", () => ({
  LanguageSwitcherCompact: () => null,
}));

// Mock notification components (deep dependency chain)
...
describe("Sidebar Component", () => {
  ...
  describe("Navigation items", () => {
    it("renders jobs link", () => {
      ...
      const jobsLinks = screen.getAllByText(/nav\.jobs/i);
    ...
    it("renders CV analysis link", () => {
      ...
      const cvLinks = screen.getAllByText(/nav\.cvAnalysis/i);
    ...
    it("renders coach link", () => {
      render(<Sidebar />);
      // Use getAllByText since there are multiple instances
      const coachLinks = screen.getAllByText(/nav\.assistant/i);
  ...
  describe("Freemium plan indicator", () => {
    it("may show premium upgrade button for free users", () => {
      ...
      const premiumButtons = screen.getAllByText(/upgradeCta/i);
