...
def _normalize_analysis_result(raw_result: Any) -> dict[str, Any]:
    ...
    if legacy_score_keys.issubset(ats_details):
        component_total = (
            min(20, max(0, _safe_int(ats_details.get("format_score"), 0)))
            + min(30, max(0, _safe_int(ats_details.get("keywords_score"), 0)))
            + min(25, max(0, _safe_int(ats_details.get("experience_score"), 0)))
            + min(15, max(0, _safe_int(ats_details.get("skills_score"), 0)))
