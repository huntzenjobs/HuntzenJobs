...
- The authoring file is environment-aware: always select `staging` or `production` explicitly before planning or applying.
- Never apply a plan that adds or destroys a service, database, volume, domain, or variable unless that change is intentional and reviewed.
