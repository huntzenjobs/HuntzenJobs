-- Les événements Stripe sont asynchrones. Cette migration rend terminaux les
-- événements d'annulation d'un utilisateur déjà supprimé et permet à
-- invoice.paid de créer atomiquement une projection manquante, uniquement si
-- l'utilisateur et la configuration de prix existent encore localement.
...
BEGIN
  ...
  IF projected_user_id IS NULL THEN
    ...
    IF NOT FOUND THEN
      -- Le compte a été supprimé après l'annulation Stripe. L'événement est
      -- authentifié, mais il ne doit recréer ni compte ni droit local.
    ...
    RETURNING free_subscription.user_id, free_subscription.plan_id, FALSE
    ...
    IF projected_user_id IS NULL THEN
      ...
      INSERT INTO public.user_subscriptions (
        ...
        stripe_price_id, metadata, updated_at
      ) VALUES (
        p_user_id, resolved_plan_id, p_status, p_period_start, p_period_end,
        p_cancel_at_period_end, p_subscription_id,
        NULLIF(BTRIM(p_customer_id), ''), p_price_id,
        jsonb_build_object('projection_source', 'stripe_subscription_updated'), NOW()
      )
  ...
  ELSE
    ...
    SET
      ...
      stripe_customer_id = COALESCE(
        NULLIF(BTRIM(p_customer_id), ''), stripe_customer_id
      ),
      stripe_price_id = p_price_id,
      plan_id = resolved_plan_id,
      current_period_start = p_period_start,
...
-- Nouvelle surcharge utilisée par le backend durci. L'ancienne reste en place
-- le temps que tous les déploiements exécutent la même version du code.
...
BEGIN
  ...
  IF p_subscription_id IS NOT NULL THEN
    ...
    IF projected_user_id IS NULL THEN
      ...
      WHERE auth_user.id = p_user_id;
      IF NOT FOUND THEN
        -- Aucun compte local à créditer. Le paiement reste journalisé et signalé
        -- à l'administration, sans notifier ni rétablir de droit utilisateur.
        INSERT INTO public.stripe_payments (
          stripe_invoice_id, user_id, stripe_customer_id, stripe_subscription_id,
          billing_reason, amount_paid, currency, interval, interval_count,
          period_start, period_end
        ) VALUES (
          p_invoice_id, NULL, p_customer_id, p_subscription_id,
          p_billing_reason, p_amount_paid, UPPER(p_currency), p_interval,
          p_interval_count, p_period_start, p_period_end
        )
      ...
      SET
        ...
        stripe_price_id = p_price_id,
        metadata = COALESCE(free_subscription.metadata, '{}'::JSONB)
          || jsonb_build_object('projection_source', 'stripe_invoice_paid'),
        updated_at = NOW()
      FROM public.subscription_plans AS free_plan
      WHERE free_subscription.user_id = p_user_id
        AND free_subscription.plan_id = free_plan.id
        ...
        AND free_subscription.status = 'active'
        AND free_subscription.stripe_subscription_id IS NULL
      RETURNING free_subscription.user_id INTO projected_user_id;

      IF projected_user_id IS NULL THEN
        IF p_subscription_status IN (
          'active', 'past_due', 'paused', 'trialing', 'incomplete'
        ) THEN
          PERFORM 1
          FROM public.user_subscriptions AS current_subscription
        ...
        INSERT INTO public.user_subscriptions (
          ...
          p_user_id, resolved_plan_id, p_subscription_status,
          p_period_start, p_period_end, p_subscription_id,
          NULLIF(BTRIM(p_customer_id), ''), p_price_id,
          jsonb_build_object('projection_source', 'stripe_invoice_paid'), NOW()
        )
        RETURNING user_id INTO projected_user_id;
      END IF;
    ELSIF p_user_id IS NOT NULL AND projected_user_id <> p_user_id THEN
      RAISE EXCEPTION 'Stripe subscription user metadata mismatch';
    END IF;
  END IF;
  ...
  INSERT INTO public.stripe_payments (
    ...
    p_invoice_id, projected_user_id, p_customer_id, p_subscription_id,
    p_billing_reason, p_amount_paid, UPPER(p_currency), p_interval,
    p_interval_count, p_period_start, p_period_end
  )
  ON CONFLICT (stripe_invoice_id) DO UPDATE SET
...
REVOKE ALL ON FUNCTION public.apply_stripe_invoice_paid(
  ...
  TIMESTAMPTZ, TIMESTAMPTZ, TEXT, INTEGER, UUID, TEXT
) FROM PUBLIC, anon, authenticated;
