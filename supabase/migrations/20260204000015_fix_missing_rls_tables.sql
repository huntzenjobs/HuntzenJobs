...
-- These legacy tables used to be created at runtime by backend/app/database.py.
-- Re-declare them here so a database can be rebuilt from migrations alone.
...
CREATE INDEX IF NOT EXISTS idx_job_search_cache_expires_at
...
CREATE INDEX IF NOT EXISTS idx_recruiter_cache_expires_at
  ON public.recruiter_cache (expires_at);
...
CREATE INDEX IF NOT EXISTS idx_user_sessions_updated_at
  ON public.user_sessions (updated_at);
