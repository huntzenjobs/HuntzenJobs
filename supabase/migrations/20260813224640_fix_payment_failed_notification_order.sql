-- Stripe peut envoyer customer.subscription.updated(past_due) avant
-- invoice.payment_failed. La notification doit rester idempotente par facture,
-- sans dépendre de l'ordre de livraison de ces deux événements.
...
BEGIN
  ...
  IF projected_user_id IS NOT NULL
    AND NOT EXISTS (
      SELECT 1
      FROM public.user_notifications
      WHERE user_id = projected_user_id
        AND type = 'payment_failed'
        AND data->>'invoice_id' = p_invoice_id
    )
  THEN
