...
Une préproduction isolée existe désormais sur Supabase, Railway, Redis, Vercel et Modal, sans copie des données de production ni secret Stripe Live. La branche Supabase persistante a reçu les 128 migrations locales et son journal local/distant est identique. Le backend Railway et le worker ARQ servent le candidat backend `072aa7d`; le frontend Vercel sert le candidat frontend `be07b7c`, qui ajoute uniquement les derniers durcissements UI/UX et leurs tests. Modal reste tagué `840bdfa`. La CI canonique du candidat backend passe entièrement, y compris l'image Docker, et les gates frontend plus récents sont verts localement et dans le build Vercel. Le frontend staging et le backend répondent `200`. Le domaine `staging.huntzenjobs.com` possède un certificat Vercel valide. La livraison email staging est verte via l'expéditeur sandbox et le sink Resend ; le transfert des domaines reste un gate de marque avant production. Le timeout réel Modal est prouvé ; le budget par environnement n'est pas activé par Modal sur ce workspace et le coût staging observé est de 0,0171 $. La capacité de lancement sans publicité est fixée à 10 VU ; le palier 50 VU reste un gate de montée marketing.
...
| Vercel | Projet `frontend-next` de l'équipe `huntzen-jobs` | Custom Environment `staging`; déploiement `dpl_BqFBfaesWHXM98MmV2UFwLUYjbZb`; domaine `staging.huntzenjobs.com`; candidat frontend `be07b7c` | Le déploiement est `READY`, aliasé et protégé par SSO. `CRON_SECRET` sensible est synchronisé avec Railway ; deux appels authentifiés au cron outbox passent et le second est dédupliqué. Next.js 16.3.1/React 19.2.8/Serwist 9.5.12 sont actifs ; le worker ne met en cache aucune API, réponse authentifiée ou origine externe, ne contient aucun chunk `_next`, n'émet aucun cookie et son précache est borné à 12 ressources publiques. Les corrections UI/UX publiques, candidat et admin sont intégrées ; le DOM staging affiche les limites de quota `5/5` et `10/10`. |
...
- Frontend : 352 tests Vitest réussis dans 53 fichiers.
...
- ESLint : 0 erreur et 99 warnings historiques ; la commande échoue dès que le seuil configuré de 102 avertissements est dépassé.
...
- Alignement staging : Vercel `READY` sur `dpl_BqFBfaesWHXM98MmV2UFwLUYjbZb` et le candidat frontend `be07b7c`; backend Railway `3b0649c3-ba80-48fe-beed-92d8c62c6b0e` et worker `9a3dc9d5-6197-4709-9d50-0960d5a9bef4` sur le candidat backend `072aa7d`; Modal canonique inchangé et tagué `840bdfa`. Health `200`, Redis `ok`, zéro 5xx observé et tâche outbox présente. Les différences entre SHA sont exclusivement frontend et ne nécessitent pas de redéploiement Railway.
