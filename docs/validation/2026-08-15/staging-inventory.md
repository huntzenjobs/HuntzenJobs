...
**Date du snapshot :** 17 août 2026
**Branche de travail :** `codex/stripe-stabilization`
**Commit distant backend testé :** `072aa7db68b535fd336ca0f0fb2f58a90d049921`
**Révision candidate distante validée en CI :** `072aa7db68b535fd336ca0f0fb2f58a90d049921`
**Révision candidate staging locale :** frontend `f3c1b38`, backend API `a4e213f`, worker `40f4aa4`
**Confidentialité :** aucune valeur de secret n'est conservée dans ce document.

## Résumé

Une préproduction isolée existe désormais sur Supabase, Railway, Redis, Vercel et Modal, sans copie des données de production ni secret Stripe Live. La branche Supabase persistante a reçu les 128 migrations locales et son journal local/distant est identique. Le backend Railway sert `a4e213f`, le worker ARQ sert `40f4aa4` et le frontend Vercel sert `f3c1b38`. Modal reste tagué `840bdfa`. La CI canonique du candidat distant passe entièrement, y compris l'image Docker ; les gates plus récents sont verts localement et dans les builds Railway/Vercel. Le frontend staging et le backend répondent `200`. Le domaine `staging.huntzenjobs.com` possède un certificat Vercel valide. La recherche Jobs V2 est désormais l'unique parcours `/jobs` et son E2E authentifié réel est vert en desktop et mobile : 7 offres, quota persistant à 1, zéro erreur console et zéro réponse HTTP en échec. La livraison email staging est verte via l'expéditeur sandbox et le sink Resend ; le transfert des domaines reste un gate de marque avant production. Le timeout réel Modal est prouvé ; le budget par environnement n'est pas activé par Modal sur ce workspace et le coût staging observé est de 0,0171 $. La capacité de lancement sans publicité est fixée à 10 VU ; le palier 50 VU reste un gate de montée marketing.
...
| Railway | Backend public version `3.0.0`, health `ok` | Environnement `staging`; backend `ravishing-reprieve` (`767356b2-2571-4e27-8246-30c7deef2861`), worker `respectful-rebirth` (`66a358fc-24df-4807-aad4-40fa70a382f4`) | Backend API `a4e213f` et worker `40f4aa4`, tous deux `SUCCESS`. Backend à 2 réplicas, runtime `/api/health/ping` `200`; aucun `ERROR`, `CRITICAL` ou traceback dans les logs après bascule. Les recherches servies depuis Redis vérifient et incrémentent désormais le quota sans bloquer l'event loop, tout en conservant la déduplication concurrente. Le palier final 10 VU produit 618 requêtes, 0 erreur, p95 109,34 ms et p99 155,18 ms. La relance 50 VU sur `3acb81c` reste rouge à 0,88 % de timeouts sans saturation CPU/mémoire ni 5xx Railway. |
| Vercel | Projet `frontend-next` de l'équipe `huntzen-jobs` | Custom Environment `staging`; déploiement `dpl_9bFsShQBt4ZMQwWCGC3FxgdwzaWk`; domaine `staging.huntzenjobs.com`; candidat frontend `f3c1b38` | Le déploiement est `READY`, aliasé et protégé par SSO. La clé de service Supabase staging invalide a été remplacée par la clé secrète dédiée `staging_backend_20260814`, sans réutiliser de clé production. `CRON_SECRET` sensible est synchronisé avec Railway ; deux appels authentifiés au cron outbox passent et le second est dédupliqué. Next.js 16.3.1/React 19.2.8/Serwist 9.5.12 sont actifs ; le worker ne met en cache aucune API, réponse authentifiée ou origine externe, ne contient aucun chunk `_next`, n'émet aucun cookie et son précache est borné à 12 ressources publiques. Jobs V2 remplace directement l'ancien parcours et la journalisation de connexion attend la propagation du JWT Supabase. |
...
## Blocages externes et gates de promotion
...
2. Obtenir la validation TVA de Leonel avant d'ouvrir les ventes ou publicités. Le sandbox Stripe Tax est `active` avec établissement Portugal, mais possède zéro registration et le code générique `txcd_10000000`; `automatic_tax` reste désactivé.
3. Rejouer immédiatement avant production le préflight `user_notifications`. Au snapshot du 14 août : 21 lignes, 49 152 octets avec index, 0 notification `payment_failed`, 0 doublon, 0 verrou en attente et index forward absent ; la création non concurrente est donc de faible risque sur ce volume observé.
4. Pour une recette par des testeurs externes, les ajouter à l'équipe Vercel ou générer un lien partagé à durée limitée ; le staging reste volontairement protégé par SSO et ne doit pas être rendu public par défaut.

La capacité initiale acceptée est 10 VU sans campagne publicitaire. Le palier 50 VU reste non reproductible à 0,88 % de timeouts et doit être repassé avant toute montée marketing. Modal staging est contrôlé par un relevé de consommation avant/après chaque campagne ; le budget par environnement est indisponible sur ce workspace et un budget global est explicitement refusé pour ne pas arrêter production.

## Préflight production lecture seule du 14 août

- PostgreSQL 17.6 ; 99 migrations appliquées, dernière version `20260325000002`.
- 29 migrations locales en attente, toutes présentes dans l'ordre forward ; aucune version appliquée en production ne manque localement.
- `user_notifications` : 21 lignes, 49 152 octets, zéro `payment_failed`, zéro doublon par facture, zéro verrou en attente, index partiel forward absent comme attendu.
- `stripe_webhook_events` existe déjà ; `stripe_effect_outbox` est absente comme attendu avant la migration.
- Le lint production expose une erreur legacy `sync_profiles_to_user_subscriptions` et trois groupes de warnings. Les migrations staging `20260810000003_supabase_lint_security_fixes.sql` et `20260812071945_fix_remaining_postgres_lint_warnings.sql` les suppriment ; le lint staging final est vide.
- Le préflight a été exécuté dans une transaction `READ ONLY`. Aucune migration, clé Test ou mutation applicative n'a touché production.

## Validation locale avant staging

- Backend : 201 tests unitaires réussis, dont la non-régression du quota sur cache hit Jobs ; 7 warnings historiques. Les validations PostgreSQL staging et la CI distante antérieures restent vertes sur leur candidat documenté.
- Ruff canonique : aucune erreur sur `backend/src`, `backend/tests` et `scripts/deployment`, avec Ruff verrouillé en `0.16.3`.
- Frontend : 359 tests Vitest réussis dans 56 fichiers.
...
- ESLint : 0 erreur et 98 warnings historiques ; la commande échoue dès que le seuil configuré de 102 avertissements est dépassé.
...
- Modal staging : application et secrets séparés, proxy auth réel vérifié, texte et PDF signé synthétiques `completed`, PDF corrompu `failed`, callbacks et nettoyages verts. La version `v4` taguée `840bdfa` répond `422` au payload vide et utilise le spawn asynchrone sans nouvel `AsyncUsageWarning`. La consommation du cycle est de 0,0171 $ ; les budgets par environnement sont indisponibles pour ce workspace, donc aucun budget global dangereux pour production n'est posé.
...
- Alignement staging : Vercel `READY` sur `dpl_9bFsShQBt4ZMQwWCGC3FxgdwzaWk` et le candidat frontend `f3c1b38`; backend Railway `767356b2-2571-4e27-8246-30c7deef2861` sur `a4e213f`; worker `66a358fc-24df-4807-aad4-40fa70a382f4` sur `40f4aa4`; Modal canonique inchangé et tagué `840bdfa`. Health `200`, zéro erreur de démarrage observée et tâche outbox présente. L'E2E final crée puis supprime un utilisateur synthétique, trouve 7 offres via Jobs V2, persiste `job_searches_used=1`, rend `/jobs`, `/cv-analysis`, `/assistant`, `/saved-jobs`, `/candidatures`, `/profile` et `/documents` en 1440×900 et 390×844 sans débordement document, erreur visible, erreur console ni réponse HTTP en échec.
