...
**Commit distant backend testé :** `3acb81c938b233bad05f8abc3100022beb617a31`
**Révision candidate validée en CI :** `3acb81c938b233bad05f8abc3100022beb617a31`
**Confidentialité :** aucune valeur de secret n'est conservée dans ce document.

## Résumé

Une préproduction isolée existe désormais sur Supabase, Railway, Redis, Vercel et Modal, sans copie des données de production ni secret Stripe Live. La branche Supabase persistante a reçu les 128 migrations locales et son journal local/distant est identique. Le frontend Vercel, le backend Railway et le worker ARQ sont publiés depuis le candidat exact `3acb81c`; Modal reste sur son déploiement fonctionnel `798b97e`. La CI canonique du candidat passe entièrement, y compris l'image Docker. Le frontend staging et le backend répondent `200`. Le domaine `staging.huntzenjobs.com` possède un certificat Vercel valide. La livraison email reste bloquée par le transfert Resend. Le timeout réel Modal est maintenant prouvé ; l'alerte budget reste à configurer. À 50 VU, une relance sur le candidat exact a reproduit 0,88 % de timeouts malgré des p95/p99 rapides.
...
| Railway | Backend public version `3.0.0`, health `ok` | Environnement `staging`; backend `ravishing-reprieve` (`32bb5890-1914-49d3-8bb2-3cd370620940`), worker `respectful-rebirth` (`1f0e3233-1a5a-4bb9-821b-83bea8398edc`) | Backend et worker issus de l'archive exacte `3acb81c`, tous deux `SUCCESS`. Backend `WORKERS=2`, healthcheck 2/2 et runtime `200`. Le worker charge sept fonctions, dont `stripe_effect_outbox_task`, et se connecte au Redis staging. La relance 50 VU produit 0,88 % de timeouts, sans 5xx ni requête Railway observée au-dessus d'une seconde. |
| Vercel | Projet `frontend-next` de l'équipe `huntzen-jobs` | Custom Environment `staging`; déploiement `dpl_BMG96D6VTexGSAHSkzUXMgyC2P4M`; domaine `staging.huntzenjobs.com` | Le déploiement du SHA exact `3acb81c` est `Ready`, aliasé et répond `200`. Next.js 16.3.1/React 19.2.8/Serwist 9.5.12 sont actifs ; le worker ne met en cache aucune API, réponse authentifiée ou origine externe et son précache est borné à 12 ressources publiques. |
...
| Modal | Deux applications historiques déployées | Environnement `staging` séparé ; application `huntzen-cv-processor-staging`, tag `798b97e`, secret staging, Proxy Token et clé Groq dédiée expirant le 11 novembre 2026 | Texte et PDF privé signé passent à `completed`; PDF corrompu à `failed`; PDF >10 Mio à HTTP 400. Deux replays réels d'une ligne `completed` la laissent strictement inchangée. Une sonde éphémère sans secret ni donnée a été annulée par Modal exactement au timeout configuré de 10 s. Toutes les données synthétiques sont nettoyées. |
...
| GitHub Actions | Workflow historique dupliqué et permissif | Workflow canonique unique `CI/CD Pipeline` | Run [`31821648854`](https://github.com/huntzenjobs/HuntzenJobs/actions/runs/31821648854) vert sur le SHA exact `3acb81c` : Backend, Frontend et image backend. Node 24, Ruff 0.16.3 et actions compatibles Node 24 sont verrouillés ; aucun échec n'est masqué et aucun test Playwright n'est exécuté. |
...
- Frontend : 319 tests Vitest réussis dans 34 fichiers.
...
- Alignement staging : Vercel, le backend Railway et le worker Railway sont sur le candidat exact `3acb81c`. Modal reste sur `798b97e`, qui contient son code fonctionnel déjà validé. Frontend `Ready`, backend et worker `SUCCESS`, health `200`, tâche outbox présente.
...
- Capacité publique staging : 10 VU vert à 0 erreur. Sur `3acb81c`, 50 VU produit 2 589 requêtes à 169,60 RPS, p95 115,11 ms et p99 132,99 ms, mais 23 timeouts (0,88 %). La limite strictement reproductible reste donc 10 VU sur `/api/health/ping`.
