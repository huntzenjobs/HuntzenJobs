...
**Commit distant backend testé :** `840bdfaf308eb0bebb9b1512511911aaf3985a5d`
**Révision candidate validée en CI :** `840bdfaf308eb0bebb9b1512511911aaf3985a5d`
**Confidentialité :** aucune valeur de secret n'est conservée dans ce document.

## Résumé

Une préproduction isolée existe désormais sur Supabase, Railway, Redis, Vercel et Modal, sans copie des données de production ni secret Stripe Live. La branche Supabase persistante a reçu les 128 migrations locales et son journal local/distant est identique. Le frontend Vercel, le backend Railway, le worker ARQ et l'application Modal canonique sont publiés depuis le candidat exact `840bdfa`. La CI canonique du candidat passe entièrement, y compris l'image Docker. Le frontend staging et le backend répondent `200`. Le domaine `staging.huntzenjobs.com` possède un certificat Vercel valide. La livraison email reste bloquée par le transfert Resend. Le timeout réel Modal est maintenant prouvé ; l'alerte budget reste à configurer. À 50 VU, une relance sur le précédent candidat applicativement équivalent pour ce chemin a reproduit 0,88 % de timeouts malgré des p95/p99 rapides.
...
| Railway | Backend public version `3.0.0`, health `ok` | Environnement `staging`; backend `ravishing-reprieve` (`2a7a05c7-e5c5-4243-b294-8c876ecc709d`), worker `respectful-rebirth` (`c3ff98d4-527e-4e81-ba3f-55c979da192a`) | Backend et worker issus de l'archive exacte `840bdfa`, tous deux `SUCCESS`. Backend `WORKERS=2`, runtime `/health` `200` avec Redis `ok`. Le worker charge sept fonctions, dont `stripe_effect_outbox_task`, et se connecte au Redis staging. La relance 50 VU sur `3acb81c` produit 0,88 % de timeouts, sans 5xx ni requête Railway observée au-dessus d'une seconde ; aucun code du chemin de charge n'a changé entre ces deux révisions. |
| Vercel | Projet `frontend-next` de l'équipe `huntzen-jobs` | Custom Environment `staging`; déploiement `dpl_J7KXN5hahccKJ6892ocjCBHQ8HqH`; domaine `staging.huntzenjobs.com` | Le déploiement du SHA exact `840bdfa` est `Ready`, aliasé et répond `200` sur `/`, `/serwist/sw.js` et `/api/public/plans`. Next.js 16.3.1/React 19.2.8/Serwist 9.5.12 sont actifs ; le worker ne met en cache aucune API, réponse authentifiée ou origine externe, ne contient aucun chunk `_next`, n'émet aucun cookie et son précache est borné à 12 ressources publiques. |
...
| GitHub Actions | Workflow historique dupliqué et permissif | Workflow canonique unique `CI/CD Pipeline` | Run [`31823727105`](https://github.com/huntzenjobs/HuntzenJobs/actions/runs/31823727105) vert sur le SHA exact `840bdfa` : Backend, Frontend et image backend. Node 24, Ruff 0.16.3 et actions compatibles Node 24 sont verrouillés ; aucun échec n'est masqué et aucun test Playwright n'est exécuté. |
...
- Backend : 189 tests réussis et 12 tests PostgreSQL staging ignorés en CI faute de variables staging ; 7 warnings historiques. La suite comprend la sécurité et la durabilité Modal, les contrats de démarrage Gunicorn, de journalisation du health ping et le contrat CI.
...
- Alignement staging : Vercel, le backend Railway, le worker Railway et l'application Modal canonique sont sur le candidat exact `840bdfa`. Frontend `Ready`, backend et worker `SUCCESS`, health `200` avec Redis `ok`, tâche outbox présente.
...
- Capacité publique staging : sur le candidat final `840bdfa`, 10 VU produit 618 requêtes à 40,68 RPS, 0 erreur, p95 111,32 ms et p99 147,23 ms. Sur `3acb81c`, 50 VU produit 2 589 requêtes à 169,60 RPS, p95 115,11 ms et p99 132,99 ms, mais 23 timeouts (0,88 %). La limite strictement reproductible reste donc 10 VU sur `/api/health/ping`.
